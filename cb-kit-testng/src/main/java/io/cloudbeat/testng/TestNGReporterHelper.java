...
public final class TestNGReporterHelper {
    ...
    public static void startBeforeClassHook(CbTestReporter reporter, ITestNGMethod testMethod) {
        if (!reporter.getInstance().isPresent())
            return;
        final String methodDisplayName = testMethod.getMethodName();
        final String methodFqn = fixFqnWithHash(testMethod.getQualifiedName());
        reporter.startSuiteHook(methodDisplayName, methodFqn, true);
    }
    public static void endBeforeClassHook(CbTestReporter reporter, ITestNGMethod testMethod) {
        if (!reporter.getInstance().isPresent())
            return;
        final String methodFqn = fixFqnWithHash(testMethod.getQualifiedName());
        reporter.endSuiteHook(methodFqn);
    }
